...
//     // Handle backend error codes inside the response
//     if (res.data?.code === 403 || res.status === 403) {
//       throw new Error(res.data?.message || "Access forbidden: Admin only");
//     }
//     if (res.data?.code === 401 || res.status === 401) {
//       throw new Error(res.data?.message || "Unauthorized: Invalid token");
//     }
...
//       err.message ||                 // JS thrown error
//       "Failed to fetch registrations."
//     );

//     // Reset stats to avoid showing broken data
//     setStats({ total: 0, approved: 0, rejected: 0, revenue: 0 });
//     setTrend([]);
//     setCountryCounts({});
//   }
...
const MainDashboard = () => {
  ...
  // --- Fetch country coordinates (using alpha endpoint) ---
  const fetchCountryCoordinates = async (code) => {
    try {
      ...
      if (res.data && res.data[0]?.latlng) {
        return res.data[0].latlng; // [lat, lng]
      }
    } catch {
      console.error(`No coordinates found for ${code}`);
    }
    return null;
  };
